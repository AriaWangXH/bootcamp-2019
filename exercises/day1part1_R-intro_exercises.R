...
gapminder07_pop_order <- gapminder07[order(gapminder07$pop, decreasing = TRUE), ]
gapminder07_pop_order[1:5,]
...
aggregate(lifeExp ~ continent, data = gapminder07, FUN = mean, na.rm=TRUE)
...
hist(gapminder07$lifeExp, 
     main = 'Distribution of life expectancy across contries in 2007 Life Expectancy', 
     xlab = 'Life expectancy')

# Bonus: Change the `breaks = ` argument from its default setting and see what happens.
hist(gapminder07$lifeExp, 
     main = 'Distribution of life expectancy across contries in 2007 Life Expectancy', 
     xlab = 'Life expectancy', breaks = 23)
...
plot(lifeExp ~ gdpPercap, data = gapminder07,
     main = 'Relationship between GDP per capita and life expectancy in 2007',
     ylab = 'Life expectacy', xlab = 'GDP per capita')

# Bonus: Add a horizontal line indicating the mean of `lifeExp` onto the plot using `abline()`.
plot(lifeExp ~ gdpPercap, data = gapminder07,
     main = 'Relationship between GDP per capita and life expectancy in 2007',
     ylab = 'Life expectacy', xlab = 'GDP per capita')
abline(h = mean(gapminder07$lifeExp), lty = 2)
